...
class Gameobj(SPRITE.Sprite):
    ...
    def rotate_center(self, im, theta):
        ...
        rotated = rotated.subsurface(rotated_rect).copy()
        return rotated
    ...
    def extract_sprites(self, spritesheet, size):
        ...
        for j in range(0, row_count):
            for i in range(0, col_count):
                new_im = pygame.Surface(size).convert()
    ...
    def update(self, dpos, world_objects, type, skip = False):
        """
...
#        assert isinstance(dpos, vec2)
        ...
        if self.state == ATTACKING and type == "projectile":
        ...
        if self.attacked == True and type == "projectile":
            if self.attack_timer == 500:
                self.attack_timer = 0
                self.attacked = False
                self.modal = False
        ...
        if self.state == ATTACKING and type == "melee":
    ...
    def draw(self):
        self.image = self.rotate_center(self.current_anim[int(self.anim_pos)], self.angle)
